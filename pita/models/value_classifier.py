...
class ValueClassifier(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.backbone = AutoModel.from_pretrained(name_or_id, trust_remote_code=True)
    ...
    def forward(
        ...
        *,
        ...
        if logit_indices is not None:
            ...
            out = self.backbone(
                input_ids=expanded_ids, attention_mask=expanded_mask, return_dict=True
            )
            ...
            return SequenceClassifierOutput(logits=cand_logits)
        last = logits[:, -1]
        if self.loss_type == "mle":
            return SequenceClassifierOutput(
